...
|---|---|
| `tests/variants.test.ts` | The WSL preset-variant transform: world rows are dropped, the WSL realm is injected, `str-replace-editor` is re-injected exactly once (and only when the source references it), Creator mode is routed through its WSL-safe Cordis adapter, prefab-family rows (`custom-bash`, `bootstrap-filesystem`) are removed, unknown rows are preserved verbatim. |
...
| `tests/client-compat.test.ts` | Browser-service compatibility across DSH releases: old `connection.api` wins even when `rpc` also exists, alpha Typert RPC calls use `/api` plus named wire arguments, legacy and alpha session-preset projections are both read, optimistic preset notes remain optional, unavailable services fail safely, and session startup falls back from legacy `workspaces` to `uiWorkspace`. |
