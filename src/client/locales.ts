...
export const zh: Record<string, string> = {
  ...
  'help.news.title': '本次更新（0.4.4）',
  'help.news.body': '复制 WSL 变体改出来的自定义模式现在能正常用了：此前生成这种预设会多出一个世界组，DSH 因行 id 重复而拒绝整个预设，选中即报 duplicate loader entry id: wsl-world。\n源预设里的 tool-str-replace-editor 行与旧的 str-replace-editor 一样会被替换（编辑器仍由 WSL 世界注入）；变体显示名不再多出一层引号。',
  ...
  'help.known.body': 'Windows 侧共享无法解析 Linux 符号链接：用 ln -s 链进工作区的项目发现不了（扫描会跳过、不报错），请把工作区注册在真实项目目录所在的层级。插件可以在自己的扫描与文件解析里用 wsl.exe 补一次 readlink，但那是新功能，尚未实现。\n会话运行中途新加入的技能不会即时出现在目录里（因为对 UNC 关闭了文件监视，见「用法与特性」）；重开一个会话即可看到。技能内容本身始终实时读取。',
...
export const en: Record<string, string> = {
  ...
  'help.news.title': "What's new in 0.4.4",
  'help.news.body': 'A custom mode built on top of a WSL variant works now: generating its preset used to append a second world group, and DSH rejects the whole preset on a duplicate row id - picking the mode failed with duplicate loader entry id: wsl-world.\nA source `tool-str-replace-editor` row is replaced like the older `str-replace-editor` one (the editor is still injected by the WSL world), and a variant display name no longer carries a doubled layer of quotes.',
  ...
  'help.known.body': 'The Windows-side share cannot resolve Linux symlinks, so a project linked in with ln -s is not discoverable (the scan skips it); register the workspace where the real project directories are. A wsl.exe readlink fallback in this plugin is possible but not implemented yet.\nA skill added while a session is running does not show up in that session\'s catalog (file watching is off for UNC workspaces, see Usage and features); start a new session to pick it up. Skill bodies are always read live.',
