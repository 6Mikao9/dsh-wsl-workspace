...
export const zh: Record<string, string> = {
  ...
  'help.known.body': 'UNC 工作区下模型收不到技能目录：宿主用 fs.watch 监视 \\\\wsl.localhost\\… 会失败（EISDIR），整条目录消息被丢弃。技能本身没问题，模型仍可按名加载。\nWindows 侧共享无法解析 Linux 符号链接：用 ln -s 链进工作区的项目发现不了（扫描会跳过、不报错），请把工作区注册在真实项目目录所在的层级。\n在 DSH 0.1.0-rc.7 / 0.1.1-rc.2 上，新建会话草稿态的预设选择器不会显示 WSL 变体（这两个版本没有草稿态 API）；进入会话后标签是正确的。\n以上都是上游或版本本身的限制，本插件无法绕过。',
...
export const en: Record<string, string> = {
  ...
  'help.known.body': 'On a UNC workspace the model does not receive the skill catalog: the host watches the share with fs.watch, which fails with EISDIR, so the whole catalog message is dropped. The skills themselves are fine and stay loadable by name.\nThe Windows-side share cannot resolve Linux symlinks, so a project linked in with ln -s is not discoverable (the scan skips it without failing); register the workspace at a level that holds the real project directories.\nOn DSH 0.1.0-rc.7 and 0.1.1-rc.2 the preset picker of a new draft session does not show the WSL variant (those releases have no draft API); the label inside the session is correct.\nAll of these are upstream or release limitations this plugin cannot work around.',
