...
- **File tools (`read`/`write`/`edit`)**: go through the Windows-side WSL 9P share; the username field does not affect them. The access mode does **not** constrain them in a WSL session. The policy is host-plane (`fs-sandbox` / `sandbox-policy` wrap the host's `fs` service, see `dsh --profile web --dump-config`), while a WSL variant deliberately mounts its own `fs` provider inside the preset's isolate realm — so the wrapper is not in the call path. Measured on `0.1.5-rc.2`: with the session on `workspace-write`, a `write` outside the workspace succeeded for both a Linux path and a `D:\...` path, with no denial. Non-WSL sessions keep the documented behaviour, since their tools consume the host `fs`. Wiring the policy into the WSL world is a follow-up; until it lands, treat a WSL session's file tools as unrestricted and the access-mode button as governing only non-WSL sessions.
- **Skill catalog**: the session's skill catalog is discovered starting at the session cwd's nearest `.git` ancestor (falling back to the cwd itself), then scanning downward for `.dsh/skills` / `.agents/skills` — including nested projects — bounded to 4 directory levels, 64 skill directories and 4096 visited directories. Register the workspace at the project root you work in; if the registered workspace itself sits inside a larger git repository, the scan starts at that repository's root (matching the host's own rule) and sibling projects may surface. A Linux symlink the Windows-side share cannot resolve is resolved through the distribution instead (`wsl.exe … readlink -f`, at most 32 per lookup, four in flight) and the walk continues at the real path, so a project linked in with `ln -s` — and nested projects below it — is discovered and deduplicated by that real path. Skill bodies always load live. The catalog itself is collected once at session start: the generated preset pins `watch: false` on the `skill-filesystem` row because watching a `\\wsl.localhost\...` path fails, so a skill added while a session is running appears in the next session.
- **bash shell lifetime**: one command per call. Every `bash` invocation is a fresh shell started in the session cwd, so `cd`, exported variables, activated virtualenvs and background jobs do not carry over between calls — chain them into one command or use absolute paths. A WSL variant has no persistent shell: the host's `persistent-shell` group registers the same `bash` tool name as the variant's own world (which would make the whole preset fail to mount) and its PTY backend refuses to start on a Windows host.
