import { defaultDistro, isAbsoluteLinuxPath, isValidWslUsername, joinUnc, listDistros, mntToWindowsPath, normalizeLinuxPath, parseWslUnc, uncToLinux } from "./wsl-CECXiGZH.js";
import { canonicalWslUnc, getWindowsWorkspace, getWorkspaceUsername, listWorkspaceKeys, registerWindowsWorkspace, setWorkspaceUsername } from "./wsl-credentials-BJXc9tfi.js";
import { resolveLinuxSymlinks } from "./links-BQxdJOMy.js";
...
function wslWorldGroup(shellPath, fsPath, includeEditor, persistent, searchPath) {
	return [
...
*/
function transformPresetForWsl(source, shellPath, fsPath, persistent, searchPath) {
	...
	result.push(wslWorldGroup(shellPath, fsPath, sawEditor, persistent, sawSearch ? searchPath : void 0));
	return result.join("\n").replace(/\n{3,}/g, "\n\n").replace(/\n+$/, "\n");
}
...
var WslSkillsProvider = class {
	...
	constructor(control, io = nodeSkillIo, now = Date.now, refreshMs = REFRESH_POLL_MS, discoveryMs = DISCOVERY_POLL_MS) {
...
async function materializeVariants(agentPresets, dshHome, paths) {
	...
	for (const preset of presets) {
		...
		try {
			...
			if (labels === void 0) {
				...
				if (match?.[1] !== void 0 && match[1].trim() !== "") name$1 = variantName(preset.id, unquoteScalar(match[1].trim()));
			}
...
function apply(ctx, config) {
	...
	if (agentPresets !== void 0) ctx.effect(() => {
		materializeVariants(agentPresets, dshHome, {
			shell: shellPath,
			fs: fsPath,
			relay: relayPath,
			node: nodePath,
