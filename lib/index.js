...
async function probeDirectory(distro, dir, depth, io) {
	...
	const markers = entries === void 0 ? allMarkers : allMarkers.filter((marker) => listed.some((entry) => entry.name === marker));
...
async function parseVariantComposition(content) {
	...
	const [include, yamlModule] = await Promise.all([import(includeSpecifier), import(yamlSpecifier)]);
