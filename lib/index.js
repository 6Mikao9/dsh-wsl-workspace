...
function wslWorldGroup(shellPath, fsPath, includeEditor, includeShellFsTools) {
	return [
		...
		"  config:",
		...includeShellFsTools ? [`    - id: shell-wsl`, `      name: '${shellPath.replace(/'/g, "''")}'`] : [],
		`    - id: fs-wsl`,
...
function transformPresetForWsl(source, shellPath, fsPath) {
	...
	const includeShellFsTools = !(sawPersistent && !hadShellTool && !hadFsTool);
	const result = [...kept];
