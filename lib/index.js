import { a as isAbsoluteLinuxPath, c as joinUnc, d as parseWslUnc, l as mntToWindowsPath, o as isValidWslUsername, r as listDistros, t as defaultDistro, u as normalizeLinuxPath } from "./wsl-qwxCKU5O.js";
import { a as registerWindowsWorkspace, i as listWorkspaceKeys, n as getWindowsWorkspace, o as setWorkspaceUsername, r as getWorkspaceUsername, t as canonicalWslUnc } from "./wsl-credentials-BwNmfX_y.js";
...
function replaceRowName(lines, span, modulePath) {
	const block = lines.slice(span.start, span.end);
...
*/
function transformPresetForWsl(source, shellPath, fsPath, cordisToolPath) {
	...
	for (const span of spans) {
		...
		if (id === "tool-cordis" && cordisToolPath !== void 0) {
...
var WslSkillsProvider = class {
	...
	async list(options) {
		...
		if (cached !== void 0) {
			...
			if (!this.refreshing.has(cacheKey)) {
				...
				this.scan(cacheKey, unc.distro, scanRoot, options.signal).catch(() => {}).finally(() => {
	...
	*/
	async scan(cacheKey, distro, scanRoot, signal) {
...
async function materializeVariants(agentPresets, dshHome, shellPath, fsPath, cordisToolPath) {
	...
	for (const preset of presets) {
		...
		const transformed = transformPresetForWsl(await agentPresets.read(preset.id), shellPath, fsPath, cordisToolPath);
...
function apply(ctx, config) {
	...
	if (agentPresets !== void 0) ctx.effect(() => {
		materializeVariants(agentPresets, dshHome, shellPath, fsPath, cordisToolPath).catch((error) => {
