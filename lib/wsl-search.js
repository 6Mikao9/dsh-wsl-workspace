import { defaultDistroSync, isAbsoluteLinuxPath, parseWslUnc } from "./wsl-CECXiGZH.js";
import { getWorkspaceUsername } from "./wsl-credentials-BJXc9tfi.js";
...
function globToRegExp(pattern) {
	return /* @__PURE__ */ new RegExp(`^${globSource(pattern)}$`);
}
/**
...
function acceptRun(run, toolName, rawOutputMaxBytes) {
	...
	if (code === 2 && INVALID_PATTERN.test(run.stderr)) throw new SearchError(`${toolName} pattern rejected by the distribution's grep${detail === "" ? "" : `: ${detail}`}`, "SEARCH_INVALID_PATTERN");
...
function registerGrep(ctx, tools, config) {
	...
	const tool = defineTool({
		...
		async execute(args, exec) {
			...
			const argv = buildWslArgv(target, GREP_SCRIPT, [
				input.pattern,
			...
			const matches = parseGrepRecords(stdout).filter((record) => {
...
function registerGlob(ctx, tools, config) {
	...
	const tool = defineTool({
		...
		async execute(args, exec) {
			...
			const argv = buildWslArgv(target, GLOB_SCRIPT, [linuxTarget(input.path), String(config.rawOutputMaxBytes + 1)]);
			const run = await runInDistro(argv, config.wslPath, config.timeoutMs, config.rawOutputMaxBytes, exec.signal);
...
var wsl_search_default = apply;

//#endregion
export { Config, apply, buildWslArgv, capMetaBytes, wsl_search_default as default, displayPath, expandBraces, globCardPage, globSearchMeta, globToRegExp, grepSearchMeta, inject, linuxTarget, matchGlob, parseGlobRecords, parseGrepRecords, registerGuidance, renderGlobText, renderGrepText, resolveTarget, retainMatches, retainPaths };
