...
`;
		...
		*/
		function apply(ctx) {
			...
			const agentPresets = ctx.get("remote.agentPresets");
			const connection = ctx.get("connection");
			const uiWorkspace = ctx.get("uiWorkspace");
			const hasUiWorkspace = uiWorkspace !== void 0;
			const hasNoteAgentPreset = typeof sessions.noteAgentPreset === "function";
			const hasRemoteNamespace = agentPresets !== void 0;
			const hasLegacyApi = connection?.api !== void 0;
			...
			const selectAgentPreset = async (sessionId, presetId) => {
				if (hasRemoteNamespace && agentPresets) return agentPresets.select(sessionId, presetId);
				if (hasLegacyApi && connection?.api) return { ok: (await connection.api.agentPresets.select({
					sessionId,
			...
			const startSessionCompat = (workspaceId) => {
				if (hasUiWorkspace && uiWorkspace) uiWorkspace.startSession(workspaceId);
				else if (typeof workspaces.startSession === "function") workspaces.startSession(workspaceId);
			};
