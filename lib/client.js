...
`;
		...
		*/
		...
		function agentPresetOf(summary) {
			if (!isRecord(summary)) return void 0;
			if (typeof summary.agentPreset === "string") return summary.agentPreset;
			return isRecord(summary.projectionValues) && typeof summary.projectionValues.agentPreset === "string" ? summary.projectionValues.agentPreset : void 0;
		}
		/** Older session stores need an optimistic note; alpha stores update from Remote events. */
		function noteAgentPresetIfAvailable(sessions, sessionId, agentPreset) {
		...
		async function selectWslAgentPreset(api, sessions, sessionId, target) {
			if (!(await api.select({
				sessionId,
				agentPreset: target
		...
		function legacyAgentPresets(connection) {
			if (!isRecord(connection.api) || !isRecord(connection.api.agentPresets)) return void 0;
			const candidate = connection.api.agentPresets;
			if (typeof candidate.list !== "function" || typeof candidate.select !== "function") return void 0;
			return candidate;
		}
		/**
		...
		*/
		function resolveAgentPresetsApi(connection) {
			if (isRecord(connection)) {
				const legacy = legacyAgentPresets(connection);
				if (legacy !== void 0) return legacy;
				const rpc = isRecord(connection.rpc) && typeof connection.rpc.call === "function" ? connection.rpc : void 0;
				if (rpc !== void 0) return {
					list: async () => ({ result: await rpc.call("/api", "agentPresets/list", { args: {} }) }),
					select: async ({ sessionId, agentPreset }) => ({ result: await rpc.call("/api", "agentPresets/select", { args: {
						agentId: sessionId,
						agentPreset
					} }) })
				};
			...
			return {
				list: async () => ({ result: {
					ok: false,
					error: { message: "agentPresets API unavailable" }
				} }),
				select: async () => ({ result: {
					ok: false,
					error: { message: "agentPresets API unavailable" }
		...
		function sessionStarter(value) {
			return isRecord(value) && typeof value.startSession === "function" ? value : void 0;
		}
		/** Start the new workspace through the service available in this DSH line. */
		function startWorkspaceSession(workspaces, uiWorkspace, workspaceId) {
		...
		function apply(ctx) {
			...
			const injected = () => ({
				...
				checkPreset: async () => {
					let roster;
					try {
						roster = (await agentPresets.list({})).result;
					} catch (error) {
			...
			ctx.effect(() => {
				...
				let defaultPreset;
				const refreshRoster = () => {
					agentPresets.list({}).then((response) => {
				...
				const maybeBind = () => {
					...
					for (const id of state.ids) {
						...
						inFlight.add(id);
						selectWslAgentPreset(agentPresets, sessions, id, target).then((selected) => {
