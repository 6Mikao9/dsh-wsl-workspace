...
`;
		...
		*/
		function apply(ctx) {
			...
			const api = rpc ? { agentPresets: {
				list: (args) => rpc.call("/agentPresets", "list", args).then((r) => ({ result: r })),
				select: (args) => rpc.call("/agentPresets", "select", args).then((r) => ({ result: r }))
			} } : connection?.api ?? { agentPresets: { list: async () => ({ result: { ok: false, value: { presets: [] }, error: { message: "agentPresets API unavailable" } } }), select: async () => ({ result: { ok: false } }) } };
			const workspaces = ctx.get("workspaces");
			const uiWorkspace = ctx.get("uiWorkspace");
			const sessions = ctx.get("sessions");
			const startSession = (workspaceId) => {
				if (typeof workspaces.startSession === "function") workspaces.startSession(workspaceId);
				else if (uiWorkspace?.startSession) uiWorkspace.startSession(workspaceId);
			};
			ensureStyles();
