...
## Нативная сборка при установке

Установка этого плагина тянет `@deepseek-ai/dsh-fs-local` (peer-зависимость), которая зависит от [`koffi`](https://www.npmjs.com/package/koffi) — динамического C FFI для Node.js. koffi во время `npm install` запускает скрипт `install` (`node ./cnoke.cjs -P . -D src/koffi --prebuild --release`):

- **Сначала пребилд**: пытается загрузить платформенный предсобранный аддон из `optionalDependencies` koffi (`@koromix/koffi-<platform>`); покрывает `win32-x64/arm64/ia32`, `linux-x64/arm64/ia32/riscv64/loong64`, `darwin-x64/arm64`, `freebsd-*`, `openbsd-*`. Если пребилд загружается — **компиляции не происходит**.
- **Компиляция fallback**: только если предсобранный аддон недоступен/не загружается, пересобирается из исходников; это требует CMake и компилятор C/C++ (на Windows предпочтителен Clang, или MinGW под MSYSTEM). Это стандартное поведение самого koffi; этот плагин сам не собирает и не содержит нативный код.

Это ожидаемо и не является вредоносным (koffi под лицензией MIT). Установка с `--ignore-scripts` пропускает выбор/сборку аддона koffi, поэтому `@deepseek-ai/dsh-fs-local` может не загрузиться на платформах без кэшированного пребилда. Сама сессия WSL не требует toolchain: инструмент bash выполняется внутри дистрибутива WSL, а файловые инструменты идут через общую папку WSL со стороны Windows.
...
- **Варианты режимов**: для каждого режима, поставляемого DSH — Standard, PTC, Minimal, Creative и экспериментальных вроде Anchored Standard — этот плагин добавляет соответствующий вариант `wsl-<режим>`. Исходные режимы остаются доступны без изменений; WSL-варианты просто запускают тот же режим в окружении выполнения WSL.

## Журнал изменений

### 0.2.4

- **Исправлено #5 — минимальный режим WSL больше не ломает цепочку рассуждений «we need/lets» первого запроса.** Раньше WSL-вариант минимального пресета (который предоставляет только `persistent-bash` + `str-replace-editor`) дополнительно внедрял одноразовый инструмент `bash` и файловые инструменты `read`/`write`/`edit`/`read_image`; дублирующееся имя инструмента `bash` и лишние схемы раздували каталог инструментов первого запроса и сбивали цепочку рассуждений. Теперь минимальные варианты сохраняют только `persistent-bash`, `str-replace-editor` и провайдер `fs-wsl`; стандартные пресеты по-прежнему получают полный набор shell + файловых инструментов.
